...
- **SSD** 和 `appendfsync always` 组合可能导致问题发生：请谨慎使用这种组合，因为该配置选项会让 Redis 每次只写入一个命令，而不像其他 `appendfsync` 选项那样一次写入多个命令，这种不断地写入少量数据的做法有可能会引发严重的写入放大问题；在某些情况下，甚至会将固态硬盘的寿命从原来的几年降低为几个月；
