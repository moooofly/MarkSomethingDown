...
根据反馈的 publish 等曲线掉底情况，运维人员进行了如下问题分析和验证：
- 在曲线掉底的时间段内，RabbitMQ 的 management 插件所维护的统计信息数据库中积压了几十万条待处理统计消息（事件）；
- 同一时间段内，业务基于打点绘出的“获取 channel 耗时“曲线飙高；
- 运维人员通过重启 RabbitMQ 的 management 插件后（等于清空积压的统计信息），统计数据库（实际上应该说成 rabbit_mgmt_db 进程）从 xg-napos-rmq-1 节点随机迁移到 xg-napos-rmq-3 节点，此时发现，整体 qps 从原来的 2000 上升到 4000 左右；此时业务获取 channel 超时时间恢复正常；
