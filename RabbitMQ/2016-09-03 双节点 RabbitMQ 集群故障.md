...
查阅 `channel_operation_timeout` 的历史，发现该参数在 3.6.1 版本引入，默认值为 **5000ms** ；之后在 3.6.2 版本作为 Bug Fixes 修正为 **15000ms** ；详情参考[这里](https://github.com/rabbitmq/rabbitmq-server/issues/667)；官方的说法为“5s is not enough for some. 15s sounds like a safer default.“；好吧，意思是崩溃的可能理论上仍旧是存在的，但被大大减小了；这应该也从侧面说明了这个异常不是恨严重吧！
...
遗留问题：
- 在限定时间内无法关闭全部 channel 的情况是否属于正常情况？
- 上面说的“某种原因“到底是什么原因？
