...
不同类型的应用，用途也不尽相同。要找到性能瓶颈，则首先需要知道系统中跑的究竟是什么类型的应用，以及它的特点是什么；
...
可能原因：**单个应用程序（或调度实体）请求了（依赖）大量 kernel 提供的服务（资源）以满足用户态的计算需要**
...
- CPU 和内存资源没有问题；
- 对系统资源有一定量需求（in）；
- 某个进程可能一直在系统态霸占着 CPU（sy 非常高，同时 cs 较低）；
...
### 案例五：wa 高 + si/so 高 + bi/bo 高 + swpd 增 + buff/cache 减
...
- **buff 逐步减少**，说明系统发现内存不足，正通过 kswapd 从 buff 那里借用部分内存（FIXME：为什么从这里借？）；
...
### 案例六：wa 高 + cs 高 + bi/bo 有值 + so 高 + swpd 增 + buff/cache 增
...
在同时打开了很多 applications 的情况下，几乎所有 memory 都被占用了；之后再启动应用时（OpenOffice），则会导致 Linux kernel 出发 swap out，将 several memory pages 换出到硬盘上的 swap file 中，以便为 OpenOffice 获取更多可用 RAM ；将 memory pages 换出到 swap file 的行为，可以通过 vmstat 的 so 列 (swap out - memory swapped to disk) 看出：


### 案例七：sy 高 + bo 有值 + cache 增 + free 减

可能原因：**获取 kernel 生成的随机数后写入本地文件**；
...
可以看到，通过 `dd` 将 kernel 生成的随机数写入本地文件时（500M 文件）：
...
### 案例八：wa 高 + bo 有值 + buff 增

可能原因：**read from `/dev/zero` and write a file, High IO Write Load**

和上一个案例相比，`dd` 改成从 `/dev/zero` 进行读取，再写入文件；而 `oflag=dsync` 的作用是令数据被直接写入磁盘，而不是仅仅写入 **page cache**；
...
可能原因：**获取 kernel 生成的随机数后写入 `/dev/null` 文件**；
...
### 案例十：sy/wa 高 + buff 减 + cache 增

可能原因：**读取本地文件写入 `/dev/null` 文件**

```shell
$ dd if=500MBfile of=/dev/null bs=1M count=500
```
